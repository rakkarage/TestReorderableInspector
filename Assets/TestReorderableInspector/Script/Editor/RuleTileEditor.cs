	...
	internal class RuleTileEditor : Editor
	{
		...
		private static void RuleInspectorOnGUI(Rect rect, RuleTile.TilingRule tilingRule)
		{
			...
			if (tilingRule.m_Output != RuleTile.TilingRule.OutputSprite.Single)
			{
				...
				EditorGUI.BeginChangeCheck();
				var newLength = EditorGUI.DelayedIntField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Sprites.Length);
				if (EditorGUI.EndChangeCheck())
		...
		public override Texture2D RenderStaticPreview(string assetPath, Object[] subAssets, int width, int height)
		{
			if (tile.m_DefaultSprite != null)
			{
				...
				if (t != null)
				{
					...
					if (method != null)
					{
						var ret = method.Invoke("RenderStaticPreview", new object[] { tile.m_DefaultSprite, Color.white, width, height });
						if (ret is Texture2D)
